...
void alarm()
{
	if(y==0)
	{
		...
		{
			char c;
			int ff=open("alarm.txt",O_RDONLY,0644);	
			cout<<"(timer) missed alarm:"<<endl;		
			while(read(ff,&c,1)!=0)
			{
			...
			cout<<endl;
			close(ff);
			ff=open("alarm.txt",O_TRUNC,0644);
			close(ff);
		}
	}
y++;
...
{
	cout<<"Recived\n";
	int ff=open("alarm.txt",O_TRUNC,0644);
	close(ff);
}
void process(int a)
{
...
int main()
{
	...
	signal(SIGTTOU,process);
	signal(SIGTTIN,process);
	signal(SIGQUIT,tab);
		...
		else
		{
			...
			if(flag==-5)
			{
				...
				if(fork()==0)
				{
					...
					if(nnn==false)
					{
						fd=open(fi,O_CREAT|O_WRONLY|O_APPEND,0644);	
						lseek(fd,0,SEEK_END);
						dup2(fd,1);
						run(s,3);
						close(fd);				
					...
					{
						fd=open(fi,O_CREAT|O_WRONLY|O_TRUNC,0644);					
						dup2(fd,1);
						run(s,3);
						close(fd);					
					}
			...
			else if(flag == -9)
			{
				...
				char al[1024]={0};
				strcpy (al,s.c_str());
				int ff=open("alarm.txt",O_WRONLY|O_CREAT|O_TRUNC,0644);
				write(ff,al,s.length());
				close(ff);
				write(fd[1],al,1024);
				close(fd[1]);
				exit(9);
